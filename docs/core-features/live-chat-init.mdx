...
- `userEmail`: Optional user email for enhanced tracking

### Example Use Case
- Start a chat for a logged-in user with their ID

### Edge Cases
- `userId` is now required when initializing the `ChatMe` SDK instance. If not provided during SDK instantiation, an error will occur.
